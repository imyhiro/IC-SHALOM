          ...
          {!isLive && (
            <>
              ...
              <div style={{
                ...
                textTransform: 'uppercase', color: '#C8956C', marginBottom: '0.5rem',
              }}>
                REUNIONES GENERALES
              </div>
    ...
    <section id="nuevo" style={{ padding: '6rem 2rem', background: t.bgAlt, transition: 'background 0.3s' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <p style={{ fontSize: '1.05rem', color: t.textSecondary, lineHeight: 1.7, maxWidth: '650px', margin: '0 auto', transition: 'color 0.3s' }}>
            ¡Qué alegría que estés en Shalom! Nos emociona recibirte y creer junto contigo que Dios tiene propósito para tu vida. Deseamos que en este lugar puedas sentir el amor de Jesús de una manera real, cercana y transformadora.
          </p>
          <p style={{ fontSize: '1.05rem', color: t.textSecondary, lineHeight: 1.7, maxWidth: '650px', margin: '1rem auto 0', transition: 'color 0.3s' }}>
            Aquí hay un espacio para ti, para crecer, conectar y caminar en familia. Es nuestro deseo que desde el primer momento te sientas en casa, amado y bienvenido.
          </p>
        </div>
...
/* ─── Involúcrate ─── */
const EQUIPOS = [
  { name: 'Equipo de música', desc: 'Para quienes desean usar su talento musical en la alabanza.' },
  { name: 'Audio', desc: 'Apoyo técnico para que cada reunión se escuche con excelencia.' },
  { name: 'Cafetería', desc: 'Servir a través de hospitalidad y dones culinarios.' },
  { name: 'Kids', desc: 'Enseñar y cuidar a las nuevas generaciones.' },
  { name: 'Bienvenida', desc: 'Recibir a cada persona con amor y calidez.' },
  { name: 'Estacionamiento', desc: 'El primer contacto para ayudar y orientar a las personas.' },
  { name: 'Proyección', desc: 'Visuales y letras que apoyan cada reunión.' },
  { name: 'Redes y Comunicación', desc: 'Capturar momentos y comunicar con creatividad.' },
  { name: 'Iluminación', desc: 'Crear atmósferas mediante iluminación creativa.' },
  { name: 'Transmisión', desc: 'Grabar y transmitir nuestra reunión de la 1 PM.' },
];

function InvolúcrateSection({ theme: t }: { theme: Theme }) {
  const [formOpen, setFormOpen] = useState(false);
  const [sending, setSending] = useState(false);
  const [sent, setSent] = useState(false);
  const [error, setError] = useState('');
  const [form, setForm] = useState({ nombre: '', edad: '', ministerio: '', correo: '', whatsapp: '' });

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.nombre || !form.whatsapp) { setError('Nombre y WhatsApp son obligatorios'); return; }
    ...
    try {
      const { error: err } = await supabase!.from('personas_nuevas').insert([{
        ...
        como_se_entero: `Involúcrate: ${form.ministerio || 'Sin especificar'}${form.edad ? ` (Edad: ${form.edad})` : ''}`,
      }]);
      if (err) { setError('Error al enviar, intenta de nuevo'); setSending(false); return; }
      setSent(true);
      setForm({ nombre: '', edad: '', ministerio: '', correo: '', whatsapp: '' });
  ...
  return (
    <section id="involucrate" style={{ padding: '6rem 2rem', background: t.bg, transition: 'background 0.3s' }}>
      <div style={{ maxWidth: '700px', margin: '0 auto', textAlign: 'center' }}>
        <p style={{ fontSize: '0.8rem', fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C8956C', marginBottom: '1rem' }}>Involúcrate</p>
        <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.75rem)', fontWeight: 700, lineHeight: 1.2, color: t.text, marginBottom: '1.5rem', transition: 'color 0.3s' }}>Involúcrate</h2>
        <p style={{ fontSize: '1.05rem', color: t.textSecondary, lineHeight: 1.7, transition: 'color 0.3s', marginBottom: '3rem' }}>
          ¡Nos emociona que quieras ser parte de Shalom! Creemos que servimos a Dios por gratitud, usando nuestros dones y talentos para impactar la vida de otros. Hay un lugar para ti.
        </p>
      </div>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem', marginBottom: '3rem' }}>
          {EQUIPOS.map(eq => (
            <div key={eq.name} style={{
              padding: '1.5rem', background: t.cardAlt, borderRadius: '1rem',
              border: `1px solid ${t.border}`, transition: 'all 0.3s', textAlign: 'center',
            }}>
              <h3 style={{ fontSize: '1rem', fontWeight: 700, color: t.text, marginBottom: '0.5rem', transition: 'color 0.3s' }}>{eq.name}</h3>
              <p style={{ color: t.textMuted, fontSize: '0.85rem', lineHeight: 1.5, transition: 'color 0.3s' }}>{eq.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ textAlign: 'center', maxWidth: '500px', margin: '0 auto' }}>
          <button
            onClick={() => { setFormOpen(!formOpen); setSent(false); }}
            style={{
                ...
                <div style={{
                  ...
                  {sent ? (
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <p style={{ color: t.textSecondary, fontSize: '0.95rem' }}>Pronto nos pondremos en contacto contigo.</p>
                      <button
                    ...
                    <>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                        <h3 style={{ fontSize: '1.15rem', fontWeight: 700, color: t.text, flex: 1, textAlign: 'center' }}>Involúcrate</h3>
                        <button onClick={() => setFormOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: t.textMuted, fontSize: '1.25rem', padding: '0.25rem' }}>&times;</button>
                      </div>
                      ...
                      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        ...
                        <select value={form.ministerio} onChange={e => setForm({ ...form, ministerio: e.target.value })} style={{ ...inputStyle, appearance: 'none', color: form.ministerio ? t.text : t.textMuted }}>
                          <option value="">Ministerio de interés</option>
                          {EQUIPOS.map(eq => <option key={eq.name} value={eq.name}>{eq.name}</option>)}
                        </select>
                        <input placeholder="Correo" type="email" value={form.correo} onChange={e => setForm({ ...form, correo: e.target.value })} style={inputStyle} />
                        <input placeholder="Teléfono WhatsApp *" type="tel" value={form.whatsapp} onChange={e => setForm({ ...form, whatsapp: e.target.value })} style={inputStyle} />
                      ...
                      <p style={{ color: t.textMuted, fontSize: '0.8rem', textAlign: 'center', marginTop: '1rem' }}>
...
function MinisteriosSection({ theme: t }: { theme: Theme }) {
  ...
  const [sent, setSent] = useState(false);
  const [error, setError] = useState('');
  const [form, setForm] = useState({ nombre: '', telefono: '', email: '', ministerio: '', mensaje: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error: err } = await supabase!.from('personas_nuevas').insert([{
        ...
        como_se_entero: `Ministerios: ${form.ministerio || 'Sin especificar'}${form.mensaje ? ` | ${form.mensaje}` : ''}`,
      }]);
      if (err) { setError('Error al enviar, intenta de nuevo'); setSending(false); return; }
      setSent(true);
      setForm({ nombre: '', telefono: '', email: '', ministerio: '', mensaje: '' });
    } catch { setError('Error al enviar, intenta de nuevo'); }
    setSending(false);
  };
  ...
  return (
    <section id="ministerios" style={{ padding: '6rem 2rem', background: t.bg, transition: 'background 0.3s' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        {/* Botón y formulario de registro */}
        <div style={{ textAlign: 'center', maxWidth: '500px', margin: '3rem auto 0' }}>
          <button
            onClick={() => { setFormOpen(!formOpen); setSent(false); }}
            style={{
              padding: '0.85rem 2.5rem', background: '#C8956C', color: '#fff',
                ...
                <div style={{
                  ...
                  {sent ? (
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <button
                        onClick={() => { setSent(false); setFormOpen(false); }}
                        style={{ marginTop: '1.5rem', padding: '0.6rem 1.5rem', background: 'none', border: `1px solid ${t.border}`, borderRadius: '999px', color: t.textSecondary, fontSize: '0.85rem', cursor: 'pointer' }}
                      >
                        Cerrar
                      </button>
                    </div>
                    ...
                    <>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                        <h3 style={{ fontSize: '1.15rem', fontWeight: 700, color: t.text, flex: 1, textAlign: 'center' }}>Regístrate</h3>
                        <button onClick={() => setFormOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: t.textMuted, fontSize: '1.25rem', padding: '0.25rem' }}>&times;</button>
                      </div>

                      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        <input placeholder="Nombre *" value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })} style={inputStyle} />
                        <input placeholder="Teléfono *" type="tel" value={form.telefono} onChange={e => setForm({ ...form, telefono: e.target.value })} style={inputStyle} />
                        <input placeholder="Email (opcional)" type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} style={inputStyle} />
                        <select value={form.ministerio} onChange={e => setForm({ ...form, ministerio: e.target.value })} style={{ ...inputStyle, appearance: 'none', color: form.ministerio ? t.text : t.textMuted }}>
                          <option value="">¿De qué ministerio quieres más información?</option>
    ...
    <section id="donativos" style={{ padding: '6rem 2rem', background: 'linear-gradient(135deg, #2d2d2d 0%, #1a1a1a 100%)', color: '#fff' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '1rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: '2.5rem' }}>
          Tu generosidad nos ayuda a seguir avanzando en la obra de Dios, alcanzando más personas y llevando esperanza a más familias. Gracias a cada donativo, podemos continuar creciendo y compartiendo el amor de Jesús con nuestra comunidad.
        </p>
